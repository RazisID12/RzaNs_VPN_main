...
ensure_agh_yaml_perms_restart() {
  ...
  # выясняем пользователя, под которым работает AdGuardHome
  local agh_user
  agh_user="$(systemctl show -p User --value AdGuardHome 2>/dev/null || true)"
  [[ -n "$agh_user" ]] || \
    agh_user="$(systemctl cat AdGuardHome 2>/dev/null \
                 | awk -F= '/^[[:space:]]*User=/{print $2; exit}' || true)"
  [[ -n "$agh_user" ]] || agh_user="adguardhome"

  chown "$agh_user:$agh_user" "$agh_yaml" 2>/dev/null || true
  chmod 640 "$agh_yaml" 2>/dev/null || true

  # если сервис падал из-за EACCES — «оживим» его (неблокирующе, с таймаутом)
  _settings__svc try-restart AdGuardHome >/dev/null 2>&1 || true
}
...
recreate(){
    ...
    if [[ -f /etc/wireguard/key && -f /etc/wireguard/rzans_svpn_main.conf && -f /etc/wireguard/rzans_fvpn_main.conf ]]; then
        ...
        echo "Server keys/configs are absent — initializing without default client."
        initWireGuard            # ▸ ключи + два server-конфига
        # никаких клиентов не добавляем
    fi
}
...
# --- выбираем адрес/домен сервера для Endpoint (через helpers из settings.sh) ---
# 1) Домен берём через server_domain: 'auto' → пусто; лишние кавычки уже убраны.
SERVER_HOST="$(server_domain)"
# 2) Внешний IPv4 берём централизованно: явный из settings или авто (ожидаем до 30 с)
SERVER_IP="$(server_ip4 30)"
if [[ "$SERVER_IP" == "0.0.0.0" ]]; then
  echo 'Global IPv4 not found after waiting 30 s' >&2
  exit 2
fi
# 3) Хост для Endpoint/имени файла: домен (если задан), иначе IP
