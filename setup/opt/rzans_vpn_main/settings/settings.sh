...
_ipset_comment_flag() {
  ...
  if _have ipset; then
    if ipset -! create __cmt_probe  hash:ip  family inet  comment >/dev/null 2>&1 \
...
agh_fix_perms() {
  ...
  # Логи
  local d="/var/log/adguardhome" l="$d/access.log"
...
# Приведение settings.yaml к форме settings_defaults.yaml:
#  • структура/комменты — из дефолтов
#  • значения — из текущих settings (кроме авто-полей)
#  • STRICT_SETTINGS=1 → удалить ключи вне схемы дефолтов (без исключений)
settings_heal() {
  local S="$SETTINGS_YAML" D="$DEFAULTS_YAML" TMP
  [[ -s "$S" ]] || {
  ...
  # 1) Собираем документ M: defaults + значения settings (без авто-полей)
  ...
  if ! yq ea -P '
    ...
    ( select(fi==1)
      | delpaths([
          ...
          ["adguard_home","version"], ["adguard_home","updated"],
          ["fail2ban","version"],     ["fail2ban","updated"]
        ])
    ) as $S |
    ($D * $S) as $M |
    # подстелим дефолты для обязательных, если пусто/""/[]/null
    def bad: . == null
             or (type=="string" and (gsub("^[[:space:]]+|[[:space:]]+$";"")== ""))
                  or (type=="array" and length==0);
    reduce ([
      ["dns","upstream"], ["dns","port_tls"],
      ["server","port_ssh"],
      ["vpn","nets","split"], ["vpn","nets","full"]
    ]) as $p ($M;
      if (getpath($p) | bad) then setpath($p; $D | getpath($p)) else . end
    )
  ' "$D" "$S" >"$TMP"; then
    rm -f "$TMP"
  ...
  # 2) Всегда чистим всё, чего нет в дефолтной схеме
  local TMP2; TMP2="$(mktemp)"
  yq ea -P '
    select(fi==0) as $D |
    select(fi==1) |
    ( def prune_like($schema):
        if (type=="object" and ($schema|type)=="object") then
          with_entries(
            select($schema | has(.key))
            | .value |= prune_like($schema[.key])
          )
        else . end;
      prune_like($D)
    )
  ' "$D" "$TMP" >"$TMP2" && mv -f "$TMP2" "$TMP"

  # 3) Комментарии: берём копию дефолта и in-place переносим ВСЕ не-object узлы из M.
  #    Это заменит скаляры и массивы, сохранив комментарии в структуре дефолта.
  local DST; DST="$(mktemp)"; cp -f "$D" "$DST"
  yq ea -i '
    # 0: dst (копия дефолта с комментариями), 1: src (готовый M)
    select(fi==0) as $dst | select(fi==1) as $src |
    reduce (
      $src
      | paths
      | select( ($src|getpath(.)) | (type != "object") )
    ) as $p (.;
      setpath($p; ($src|getpath($p)))
    )
  ' "$DST" "$TMP"
...
yaml_allow_all() {
  ...
  _yaml_merged | yq e -r '
    def norm(x): if x == null then [] elif (x|type)=="array" then x else [x] end;
    ( norm(.allowip.ipv4) + norm(.allowip.ipv6) )
...
server_iface() {
  ...
  ifc="$(printf '%s' "$ifc" | sed -E 's/^[[:space:]]+//; s/[[:space:]]+$//')"
  # поддержим 'auto' как явный сигнал автодетекта
  if [[ ${ifc,,} == auto || -z "$ifc" || ! -e "/sys/class/net/$ifc" ]]; then
    ifc="$(_primary_iface 30)"
  fi
  printf '%s' "$ifc"
}
...
agh_heal() {
  ...
  # 4. Дозаполняем ТОЛЬКО «плохие» поля из патча (без форса)
  #    (пустая строка, null, пустой массив считаются «плохими»).
  local FIX; FIX="$(mktemp)"
  yq ea -P '
    select(fi==0) as $M |
    select(fi==1) as $PATCH |
    def bad_str(x):  (x == null) or ((x|type) != "string") or ((x|type)=="string" and (x|gsub("^[[:space:]]+|[[:space:]]+$";"")== ""));
    def bad_arr(x):  (x == null) or ((x|type) != "array")  or ((x|type)=="array"  and (x|length)==0);
    . = $M
    # дозаполняем только если «плохо»
    | ( if bad_arr(.dns.upstream_dns)  then .dns.upstream_dns  = $PATCH.dns.upstream_dns  else . end )
    | ( if bad_arr(.dns.bootstrap_dns) then .dns.bootstrap_dns = $PATCH.dns.bootstrap_dns else . end )
    | ( if bad_arr(.dns.fallback_dns)  then .dns.fallback_dns  = $PATCH.dns.fallback_dns  else . end )
    | ( if bad_str(.http.address)      then .http.address      = $PATCH.http.address      else . end )
    | ( if bad_arr(.dns.bind_hosts)    then .dns.bind_hosts    = $PATCH.dns.bind_hosts    else . end )
  ' "$MERGE_TMP" "$PTMP" >"$FIX" && mv -f "$FIX" "$MERGE_TMP"
  rm -f "$PTMP"

  # 5. Комментарии: переносим значения из MERGE поверх копии base (in-place)
...
apply_keys() {
  ...
  raw="${raw//,/ }" 
   ...
   for _tok in $raw; do
     [[ -z "${_tok:-}" ]] && continue
     if [[ -z "${_seen[$_tok]:-}" ]]; then _seen["$_tok"]=1; _keys+=("$_tok"); fi
   done
