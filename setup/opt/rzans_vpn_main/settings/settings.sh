...
settings_heal() {
  ...
  # 1) Санитизируем settings: для критичных путей пустые значения считаем «отсутствующими»
  #    (null/""/[] → delpath), чтобы при merge подставились дефолты.
  local S_SAN
  S_SAN="$(mktemp)"
  # Без def/try/getpath — совместимо с любым yq v4 (gojq)
  if ! yq e -P '
    del( .dns.upstream?   | select(. == null or . == "" or (type == "array" and length == 0)) ) |
    del( .dns.port_tls?   | select((tostring) == "") ) |
    del( .vpn.nets.split? | select((tostring) == "") ) |
    del( .vpn.nets.full?  | select((tostring) == "") )
  ' "$S" >"$S_SAN"; then
    rm -f "$S_SAN"
    echo "ERROR: settings_heal: sanitize failed" >&2
    return 1
  fi

  # 2) Собираем документ M: defaults + очищённые settings
  TMP="$(mktemp)"
  if ! yq ea -P 'select(fi==0) * select(fi==1)' "$D" "$S_SAN" >"$TMP"; then
    rm -f "$TMP" "$S_SAN"
