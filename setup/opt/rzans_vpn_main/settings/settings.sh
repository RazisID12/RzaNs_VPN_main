...
settings_heal() {
  ...
  # 1b) Дозаполнить критичные ключи, если они пусты/null/[] — значениями из defaults
  for k in dns.upstream dns.port_tls server.port_ssh vpn.nets.split vpn.nets.full; do
    cur="$(yq e -r ".${k} // \"__absent__\"" "$TMP" 2>/dev/null || echo '__absent__')"
    cur_trim="$(printf '%s' "$cur" | sed -E 's/^[[:space:]]+//; s/[[:space:]]+$//')"
    if [[ "$cur_trim" == "__absent__" || "$cur_trim" == "null" || "$cur_trim" == "[]" || -z "$cur_trim" ]]; then
      def_json="$(yq e -r ".${k} | @json" "$D" 2>/dev/null || echo 'null')"
      tmp2="$(mktemp)"
      KEY="$k" VAL="$def_json" \
      yq e -P 'setpath( env(KEY)|split("."); (env(VAL)|from_yaml) )' \
        "$TMP" >"$tmp2" || { rm -f "$tmp2" "$TMP"; echo "ERROR: settings_heal: YAML merge failed" >&2; return 1; }
      mv -f -- "$tmp2" "$TMP"
    fi
  done

  # 2) (убрано) Явная очистка лишних ключей не нужна: далее вливаем значения
  #     поверх копии дефолтов только в те места, что есть в схеме.

  # 3) Перенос значений без reduce/def:
  #    копируем ТОЛЬКО листья из M в существующие пути копии дефолтов (DST).
  #    Комментарии из дефолтов сохраняются, т.к. редактируем DST по месту.
  local DST; DST="$(mktemp)"; cp -f "$D" "$DST"
  # Собираем список путей-листьев в формате JSON-массивов (["a","b",0,"c"])
  local -a _paths=()
  mapfile -t _paths < <(
    yq e -o=json -I=0 '
      paths
      | select( (getpath(.)) | (type != "object") )
    ' "$TMP" 2>/dev/null || true
  )
  if ((${#_paths[@]})); then
    local PJSON VAL tmp2 _exists
    for PJSON in "${_paths[@]}"; do
      # переносим только если путь существует в DST (схема дефолтов)
      _exists="$(
        P="$PJSON" yq e -r '
          try getpath(env(P)|from_json) catch "__missing__"
        ' "$DST" 2>/dev/null || echo "__missing__"
      )"
      [[ "$_exists" == "__missing__" ]] && continue
      # берём значение из M по этому пути и устанавливаем его в DST
      VAL="$(P="$PJSON" yq e -o=json -I=0 'getpath(env(P)|from_json)' "$TMP" 2>/dev/null || echo 'null')"
      tmp2="$(mktemp)"
      P="$PJSON" V="$VAL" yq e -P '
        setpath( env(P)|from_json; (env(V)|from_json) )
      ' "$DST" >"$tmp2" || { rm -f "$tmp2"; continue; }
      mv -f -- "$tmp2" "$DST"
    done
  fi
