...
#    (цепочку Mapping удаляем после flush’а)
ipt -t nat -F
...
# IPv6 — только если ip6tables доступна; выполняем полный flush nat, затем удаляем цепочку RZANS_VPN_MAIN-MAPPING
if [[ "$HAS_IP6" == y ]]; then
...
# ── 5. сохраняем ipset-баны и кастомный хук ─────────────────────────
if [[ "$HAS_IPSET" == y ]]; then
