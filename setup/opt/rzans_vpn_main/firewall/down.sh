...
# Снять ровно наши отложенные fallback-джобы (очередь Z, с меткой)
if command -v at >/dev/null 2>&1 && command -v atrm >/dev/null 2>&1; then
  AT_SIG="RZANS_FALLBACK_rzans_vpn_main"
  for jid in $(at -q Z -l 2>/dev/null | awk '$1 ~ /^[0-9]+$/ {print $1}'); do
    at -c "$jid" 2>/dev/null | grep -Fq "$AT_SIG" && atrm "$jid" || true
  done
fi
...
read_settings() {
  ...
  # сети для MASQUERADE, как в up.sh (нужны всегда)
...
# SNAT: снять ровно наши правила по комменту
del_by_comment nat POSTROUTING POSTROUTING 'RZANS_SNAT'

# MASQUERADE: убрать ровно наши правила
del_by_comment nat POSTROUTING POSTROUTING 'RZANS_MASQ'

# (если старые версии добавляли FORWARD-правила прямо в базовую цепь — снимем их)
for NET in "$SVPN_NET4" "$FVPN_NET4"; do
  [[ -n "$NET" ]] || continue
  del  filter FORWARD -s "$NET" -j ACCEPT
  del  filter FORWARD -d "$NET" -j ACCEPT
done
