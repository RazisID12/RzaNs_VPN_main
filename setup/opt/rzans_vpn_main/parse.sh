...
# ── читаем флаг route_all (синхронизирован с update.sh / settings.sh) ───────
. /opt/rzans_vpn_main/settings/settings.sh
ROUTE_ALL="$(yaml_bool 'routing.route_all' n)"

#+ выберем бинарь для IDN (idn или idn2)
IDN_BIN=""
if command -v idn >/dev/null 2>&1; then IDN_BIN="idn"; elif command -v idn2 >/dev/null 2>&1; then IDN_BIN="idn2"; fi
IDN_TLD='--no-tld'
...
if [[ -z "${1:-}" || "$1" == "host" || "$1" == "hosts" ]]; then
    ...
    cut -d ';' -f 2 download/dump.csv | \
      ...
      { [[ -n "$IDN_BIN" ]] && CHARSET=UTF-8 "$IDN_BIN" $IDN_TLD || cat; } >> temp/include-hosts.txt
